...
let nodesWithDeps = new Set();
let groupCounts = {};

// Helper: extract color string from group info
// Handles both string format ("LightCoral") and object format ({"color": "lightcoral"})
function getGroupColor(groupInfo) {
    if (!groupInfo || !groupInfo.color) return 'lightgray';
    if (typeof groupInfo.color === 'string') return groupInfo.color;
    if (typeof groupInfo.color === 'object') return groupInfo.color.color || 'lightgray';
    return 'lightgray';
}

// Helper: extract font color from group info
function getGroupFontColor(groupInfo) {
    if (!groupInfo) return 'black';
    // Check color.font.color (schema format)
    if (groupInfo.color && typeof groupInfo.color === 'object' && groupInfo.color.font) {
        return groupInfo.color.font.color || 'black';
    }
    // Check top-level font.color (legacy format)
    if (groupInfo.font) return groupInfo.font.color || 'black';
    return 'black';
}
